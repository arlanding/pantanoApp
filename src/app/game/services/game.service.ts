...
export class GameService {
  ...
  getGame(numberOfQuestionsRequired) {
    ...
    return this.http.get('https://patanoapp.com.ar/api/pantanogame/questions/10');
  }

  postUserMatchData(matchData: GameConfig) {
    const gameResult = matchData.win ? 'win' : 'lose';
    const params = {
      "nickname": matchData.userData.nickname,
      "email": matchData.userData.email,
      "instagram_user": matchData.userData.instagram,
      "questions": matchData.userAnswers,
      "gameResult": gameResult
    }
    return this.http.post('https://patanoapp.com.ar/api/gameresult', params);
  }
}
